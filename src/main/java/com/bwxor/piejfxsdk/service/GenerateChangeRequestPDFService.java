...
public class GenerateChangeRequestPDFService {
    ...
    public boolean generatePdf(String outputFolderPath) {
        try (PDDocument document = new PDDocument()) {
            ...
            addHeaderPage(document, doc);
            addDetailsPage(document, doc);

            closeCurrentStream();

            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss");
            String timestamp = LocalDateTime.now().format(formatter);
            Path fullPath = Paths.get(outputFolderPath, "change-request-" + timestamp + ".pdf");
            document.save(fullPath.toFile());
            return true;
        } catch (IOException e) {
            throw new RuntimeException("Failed to generate Change Request PDF", e);
        }
    }

    private void addHeaderPage(PDDocument document, ChangeRequestDocument doc) throws IOException {
        createNewPage(document);
        writeText(document, "Change Request", 12, false, 35f);
        for (String line : wrapText(doc.changeId() + " — " + doc.title(), fontBold, 24, MAX_TEXT_WIDTH)) {
            writeText(document, line, 24, true, 34f);
        }
        currentY -= 20f;
        if (notBlank(doc.requestedBy())) writeParagraph(document, "Requested By: " + doc.requestedBy(), 12, false, LINE_HEIGHT + 4f);
        if (notBlank(doc.requestDate()))  writeParagraph(document, "Request Date: " + doc.requestDate(), 12, false, LINE_HEIGHT + 4f);
        if (notBlank(doc.targetVersion())) writeParagraph(document, "Target Version: " + doc.targetVersion(), 12, false, LINE_HEIGHT + 4f);
        if (notBlank(doc.priority()))     writeParagraph(document, "Priority: " + doc.priority(), 12, false, LINE_HEIGHT + 4f);
        if (notBlank(doc.status()))       writeParagraph(document, "Status: " + doc.status(), 12, false, LINE_HEIGHT + 4f);
        if (notBlank(doc.approvedBy()))   writeParagraph(document, "Approved By: " + doc.approvedBy(), 12, false, LINE_HEIGHT + 4f);
        if (notBlank(doc.approvalDate())) writeParagraph(document, "Approval Date: " + doc.approvalDate(), 12, false, LINE_HEIGHT + 4f);
    }

    private void addDetailsPage(PDDocument document, ChangeRequestDocument doc) throws IOException {
        createNewPage(document);

        if (notBlank(doc.description())) {
            writeText(document, "Description", 16, true, 24f);
            writeParagraph(document, doc.description(), 12, false, LINE_HEIGHT);
            currentY -= SUBCATEGORY_SPACING;
        }

        if (notBlank(doc.justification())) {
            writeText(document, "Justification / Business Case", 16, true, 24f);
            writeParagraph(document, doc.justification(), 12, false, LINE_HEIGHT);
            currentY -= SUBCATEGORY_SPACING;
        }

        if (doc.impactedComponents() != null && !doc.impactedComponents().isEmpty()) {
            writeText(document, "Impacted Components", 16, true, 24f);
            for (ChangeRequestImpactItem item : doc.impactedComponents()) {
                writeParagraph(document, item.component() + ": " + item.description(), 12, false, LINE_HEIGHT);
            }
            currentY -= SUBCATEGORY_SPACING;
        }

        if (notBlank(doc.testingStrategy())) {
            writeText(document, "Testing Strategy", 16, true, 24f);
            writeParagraph(document, doc.testingStrategy(), 12, false, LINE_HEIGHT);
            currentY -= SUBCATEGORY_SPACING;
        }

        if (notBlank(doc.rollbackPlan())) {
            writeText(document, "Rollback Plan", 16, true, 24f);
            writeParagraph(document, doc.rollbackPlan(), 12, false, LINE_HEIGHT);
        }
    }
    ...
    private void createNewPage(PDDocument document) throws IOException {
        ChangeRequestDocumentState state = ChangeRequestDocumentState.instance;
        closeCurrentStream();
